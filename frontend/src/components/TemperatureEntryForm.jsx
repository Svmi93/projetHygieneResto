...
const TemperatureEntryForm = ({ onRecordAdded, userRole }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // L'ID de l'utilisateur connecté (employé)
    ...
    const recordData = {
      ...
      timestamp,
      notes,
      // user_id pour les employés. Pour les autres rôles, le backend pourrait le déduire du token.
      user_id: userRole === 'client' ? parseInt(userId) : undefined
    };
    ...
    try {
      ...
      if (userRole === 'client') { // Pour les employés
        // Le chemin est relatif à la baseURL de axiosInstance ('http://localhost:5001/api')
        endpoint = '/client/temperatures';
      ...
      const response = await axiosInstance.post(endpoint, recordData);
