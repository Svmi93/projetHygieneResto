...
exports.addTemperatureRecordByAdminClient = async (req, res) => {
    ...
    try {
        ...
        const [result] = await pool.execute(
            'INSERT INTO temperature_records (user_id, siret_etablissement, type, location, temperature, timestamp, temperature_type, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', // <--- ADDED timestamp, temperature_type
            [user_id, employeeSiret, type, location, temperature, timestamp, temperature_type, notes] // <--- ADDED timestamp, temperature_type
        );

        const newRecord = { id: result.insertId, user_id, siret_etablissement: employeeSiret, type, location, temperature, timestamp, temperature_type, notes }; // <--- ADDED timestamp, temperature_type
        res.status(201).json(newRecord);
    } catch (error) {
